...
import { CONTRACT_ADDRESSES, TOKENS } from "../utils/index.js";
import { createMockProvider, createMockSigner, MOCK_ADDRESSES } from "./test-utils.js";
describe('PaymentsService', () => {
    ...
    describe('Token operations', () => {
        ...
        it('should throw for unsupported token in allowance', async () => {
            try {
                await payments.allowance('0x123', 'FIL');
                assert.fail('Should have thrown');
            }
            catch (error) {
                assert.include(error.message, 'not supported');
        ...
        it('should throw for unsupported token in approve', async () => {
            try {
                await payments.approve('0x123', 100n, 'FIL');
                assert.fail('Should have thrown');
            }
            catch (error) {
                assert.include(error.message, 'not supported');
    ...
    describe('Service approvals', () => {
        ...
        it('should approve service as operator', async () => {
            ...
            const tx = await payments.approveService(serviceAddress, rateAllowance, lockupAllowance, 86400n);
            assert.exists(tx);
            assert.exists(tx.hash);
            assert.typeOf(tx.hash, 'string');
        });
        ...
        it('should throw for unsupported token in service operations', async () => {
            try {
                await payments.approveService(serviceAddress, 100n, 1000n, 86400n, 'FIL');
                assert.fail('Should have thrown');
            }
            catch (error) {
                assert.include(error.message, 'not supported');
    ...
    describe('Error handling', () => {
        it('should throw errors from payment operations', async () => {
            ...
            const errorPayments = new PaymentsService(errorProvider, errorSigner, mockPaymentsAddress, mockUsdfcAddress, false);
            try {
    ...
    describe('Deposit and Withdraw', () => {
        ...
        it('should throw for unsupported token in deposit', async () => {
            try {
                await payments.deposit(ethers.parseUnits('100', 18), 'FIL');
                assert.fail('Should have thrown');
            }
            catch (error) {
        ...
        it('should throw for unsupported token in withdraw', async () => {
            try {
                await payments.withdraw(ethers.parseUnits('50', 18), 'FIL');
                assert.fail('Should have thrown');
            }
            catch (error) {
                assert.include(error.message, 'Unsupported token');
            ...
            it('should calculate available funds correctly with time-based lockup', async () => {
                ...
                mockProvider.call = async (transaction) => {
                    ...
                    if (data != null && data.includes('ad74b775') === true) {
                        ...
                        const lockupRate = ethers.parseUnits('0.1', 18);
                        const lockupLastSettledAt = 1000000 - 100;
