...
import { calculateLastProofDate, createError, epochToDate, getCurrentEpoch, SIZE_CONSTANTS, TIMING_CONSTANTS, timeUntilEpoch, } from "../utils/index.js";
...
export class StorageContext {
    ...
    constructor(synapse, warmStorageService, provider, dataSetId, options) {
    ...
    static async create(synapse, warmStorageService, options = {}) {
        ...
        if (resolution.dataSetId === -1 || options.forceCreateDataSet === true) {
            finalDataSetId = await StorageContext.createDataSet(synapse, warmStorageService, resolution.provider, options.withCDN ?? false, options.callbacks);
        }
        else {
    ...
    static async createDataSet(synapse, warmStorageService, provider, withCDN, callbacks) {
    ...
    static async resolveProviderAndDataSet(synapse, warmStorageService, providerResolver, options) {
        ...
        if (options.providerId != null) {
            return await StorageContext.resolveByProviderId(signerAddress, options.providerId, options.withCDN ?? false, warmStorageService, providerResolver);
        }
        if (options.providerAddress != null) {
            return await StorageContext.resolveByProviderAddress(options.providerAddress, warmStorageService, providerResolver, signerAddress, options.withCDN ?? false);
        }
        return await StorageContext.smartSelectProvider(signerAddress, options.withCDN ?? false, warmStorageService, providerResolver, signer);
    }
    ...
    static async resolveByProviderId(signerAddress, providerId, withCDN, warmStorageService, providerResolver) {
        ...
        const providerDataSets = dataSets.filter((ps) => ps.providerId === provider.id && ps.isLive && ps.isManaged && ps.withCDN === withCDN);
    ...
    static async resolveByProviderAddress(providerAddress, warmStorageService, providerResolver, signerAddress, withCDN) {
        ...
        return await StorageContext.resolveByProviderId(signerAddress, provider.id, withCDN, warmStorageService, providerResolver);
    }
    static async smartSelectProvider(signerAddress, withCDN, warmStorageService, providerResolver, signer) {
        const dataSets = await warmStorageService.getClientDataSetsWithDetails(signerAddress);
    ...
    static async performPreflightCheck(size, withCDN, warmStorageService, paymentsService) {
    ...
    async preflightUpload(size) {
        const preflightResult = await StorageContext.performPreflightCheck(size, this._withCDN, this._warmStorageService, this._synapse.payments);
        return {
            ...preflightResult,
    ...
    async _processPendingPieces() {
        ...
        try {
            ...
            const addPiecesResult = await this._pdpServer.addPieces(this._dataSetId, addPiecesInfo.clientDataSetId, addPiecesInfo.nextPieceId, pieceDataArray);
